...
async function cut(lang) {
  let reading
  let localFile
  const langPath = config.get(`${lang}.langPath`)
  const yearWeek = moment()
    .add(7, 'days')
    .format('WW')
  ...
  try {
    ...
    $1('p.sb')
      .last()
      .append(`<br><br><br><div class="research-title"><b>${langResearchesTitle}</b></div><br>`)
    ...
    for (let linksIndex = 0; linksIndex < links.length; linksIndex++) {
      ...
      console.log(
        'progress:',
        require('util').inspect(`${linksIndex + 1} / ${links.length}`, { colors: true, depth: 0 }),
      )
      ...
      if (Array.isArray(references)) {
        const [researchRefs, versesRefs] = _.partition(
          references,
          ref => ref.articleClasses && ref.articleClasses.includes('scriptureIndexLink'),
        )
                ...
                publications.forEach(publication => {
                  ...
                  if (content.length > 1000) {
                    content = `${shorten(
                      content,
                      800,
                    )} <a class="link" href=${naturalHref} target="_blank">...<img src="http://jwreading.com/img/external-link.png" style="width: 16px;height: 16px;position: relative;top: -1px;left: 5px;"/></a><br>`
                  }
        ...
        if (versesRefs) {
          versesRefs.forEach(
            verseRef =>
              (referencesText += `<br><b>${verseRef.title}</b><br>${verseRef.content.replace(/ class="v"/gm, '')}`),
          )
        }
      } else {
        // `references` is an object containing a note
      ...
      if (src) {
        $3('figure')
          .children('img')
          .attr('src', `${domainRoot}${src}`)
        $3('figure')
          .children('img')
          .attr('style', 'width: unset; height: unset;')
      ...
      $1('body').append(`
        <p>
          <b></b><a href="#link${linksIndex}" id="${linksIndex}" class="research">${
        classes.includes('cl') ? langChapter + ' ' + text : text
      }</a></b>
          <br>${$3.html()}
        </p>
      `)
    ...
    $1('body').attr(
      'style',
      'color:#505D6E;font-family:Helvetica, Arial, sans-serif;font-weight:normal;font-size:16px;',
    )
    $1('body').prepend(
      `<br><p></p><b><span id="weeklyPortion">${langReadingSentence}&nbsp;${readingPortion}.</span></b></p><br>`,
    )
    $1('body').prepend(
      `<p style="background:#505D6E;padding:20px;padding-bottom:30px;color:#FFFFFF;"><a href="http://www.jwreading.com" style="color: #FFFFFF;text-decoration:none"><img src="http://www.jwreading.com/assets/images/book.png" style="width: 32px; height: 32px;position: relative; top: 10px; left: 3px;"/>&nbsp;&nbsp;&nbsp;&nbsp;JW Reading - ${langPortionHeader}</a></p>`,
    )
    ...
    reading = $1.html()
    localFile = `public/portions/${langPath}/${yearWeek}dbr11.html`
    const remoteFile = `/www/portions_${langPath}/${yearWeek}dbr11.html`
    await writeFileAsync(localFile, reading)
    ftp(localFile, remoteFile)
  } catch (error) {
